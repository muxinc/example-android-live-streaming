...
public class SrsFlvMuxer {
     ...
     */
    public void addTrack(boolean isVideo, MediaFormat format) {
        if (isVideo) {
     ...
     * @param isVideo The track index for this sample.
     * @param byteBuf The encoded sample.
     * @param bufferInfo The buffer information related to this sample.
     */
    public SrsFlvFrame writeSampleData(boolean isVideo, ByteBuffer byteBuf, MediaCodec.BufferInfo bufferInfo) {
        if (isVideo) {
            return flv.writeVideoSample(byteBuf, bufferInfo);
        } else {
            return flv.writeAudioSample(byteBuf, bufferInfo);
        }
    }

    public void releaseFrame(SrsFlvFrame frame) {
        if (frame.isVideo()) {
    ...
    private class SrsFlv {
        ...
        public SrsFlvFrame writeAudioSample(final ByteBuffer bb, MediaCodec.BufferInfo bi) {
            ...
            return writeRtmpPacket(SrsCodecFlvTag.Audio, dts, 0, aac_packet_type, audio_tag);
        }
        ...
        public SrsFlvFrame writeVideoSample(final ByteBuffer bb, MediaCodec.BufferInfo bi) {
            ipbs.clear();
...
//                    writeH264SpsPps(dts, pts);
                ...
                if (!frame_pps.data.equals(h264_pps)) {
                    ...
                    return writeH264SpsPps(dts, pts);
                }
                return null;
            ...
            //writeH264SpsPps(dts, pts);
            return writeH264IpbFrame(ipbs, type, dts, pts);
        }

        private SrsFlvFrame writeH264SpsPps(int dts, int pts) {
            ...
            return writeRtmpPacket(SrsCodecFlvTag.Video, dts, frame_type, avc_packet_type, video_tag);
        }

        private SrsFlvFrame writeH264IpbFrame(ArrayList<SrsFlvFrameBytes> frames, int type, int dts, int pts) {
            ...
            // the timestamp in rtmp message header is dts.
            return writeRtmpPacket(SrsCodecFlvTag.Video, dts, type, SrsCodecVideoAVCType.NALU, video_tag);
        }

        private SrsFlvFrame writeRtmpPacket(int type, int dts, int frame_type, int avc_aac_type, SrsAllocator.Allocation tag) {
